...
class PackedDatasetIterator:
    ...
    def __next__(self):
        ...
        chunk_id = block_idx // self._n_blocks  # type: ignore
        buffer = self._buffers[chunk_id]
        elem_id = (block_idx % self._n_blocks) * self._block_size  # type: ignore
...
class CombinedDataset(IterableDataset):
    def __init__(self, datasets, seed, weights=None, max_num_samples=None):
        self._seed = seed
    ...
    def __iter__(self):
        return CombinedDatasetIterator(self._datasets, self._seed, self._weights, self._max_num_samples)
...
class CombinedDatasetIterator:
    def __init__(self, datasets, seed, weights, max_num_samples):
    ...
    def __next__(self):
        ...
        if self._max_num_samples is not None:
            if worker_id != 0:
                if self._index >= self._max_num_samples // num_workers:
                    raise StopIteration
            else:
                if self._index >= self._max_num_samples - (self._max_num_samples // num_workers) * (num_workers - 1):
                    raise StopIteration
            # if self._index * num_workers + worker_id >= self._max_num_samples:
            #     return StopIteration
        self._index += 1
        (dataset,) = self._rng.choices(self._datasets, weights=self._weights, k=1)
        # from transformers import AutoTokenizer

        # tokenizer = AutoTokenizer.from_pretrained("ckpts/tnl")
        # input_ids = next(dataset)["input_ids"]
